...
#define rall(x) rbegin(x), rend(x)
    ...
    constexpr int FACT_SIZE = 3000010;
...
int main()
{
    ...
    while (tt--)
    {
        int a, b, c, k;
        cin >> a >> b >> c >> k;
        auto gda = gcd_distribution(a);
        for (auto &[g, _] : gda)
            g = a / g;
        auto gdb = gcd_distribution(b);
        for (auto &[g, _] : gdb)
            g = b / g;
        auto gdc = gcd_distribution(c);
        for (auto &[g, _] : gdc)
            g = c / g;
        static array_map<mint, 3000010> mp;
        mp.clear();
        for (auto [la, ca] : gda)
        {
            for (auto [lb, cb] : gdb)
