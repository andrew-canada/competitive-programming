...
void solve()
{
    ...
    rep(i, 0, K) cin >> D[i];
    ll G = 0;
    rep(i, 0, K) G = gcd(G, D[i]);
    unordered_map<int, int> n;
    for (auto x : D)
        n[x]++;
    unordered_map<int, int> m;
    auto f = [&](ll a) -> vector<pair<int, int>>
    {
        /*
        auto DIV = Divisors(a);
        int len = DIV.size();
        vector<pair<int, int>> res(len);
        rep(i, 0, len) res[i] = {a / DIV[i], a / DIV[i]};
        for (int i = len - 1; i >= 0; i--){
            rep(j, i + 1, len) if (DIV[j] % DIV[i] == 0){
        ...
        auto g = [&](int c) -> void
        {
            ...
            rep(i, 0, len * c)
            {
                p.push_back({p[i].first * pri, p[i].second * pri});
            }
            for (int i = len * c - 1; i >= 0; i--)
            {
                p[i + len].second -= p[i].second;
            }
        };
        while (pri * pri <= a)
        {
    ...
    for (auto a : Divisors(G))
        if (m.count(a))
        {
            int b = m[a];
            if (b == 0)
                continue;
            mint tmp = table.fact(A * B * C / a);
            for (auto [c, d] : n)
                tmp *= table.invfact(c / a).pow(d);
            ans += tmp * b;
        }
    cout << (ans * table.inv(A * B * C)).val() << "\n";
}
